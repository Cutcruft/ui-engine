...
async function ensureTiptap() {
    if (loaded)
        return;
    if (document.querySelector('link[href*="tiptap"]')) {
        loaded = true;
        return;
    }
    ...
    mount(container, props, onEvent) {
        ...
        try {
            ...
            if (Editor && StarterKit) {
                ...
                const placeholderExt = window.tiptapPlaceholder;
                if (placeholderExt)
                    extensions.push(placeholderExt.configure({ placeholder: props.placeholder || "" }));
                editor = new Editor({
                    element: editorEl,
                    extensions,
                ...
                if (toolbar) {
                    ...
                    mkBtn("U", "Underline", () => {
                        const ext = window.tiptapUnderline;
                        if (ext)
            ...
            update(newProps) {
                if (editor && newProps.value !== undefined) {
                    ...
                    if (newProps.value !== current) {
                        if (editor.commands)
                            editor.commands.setContent(newProps.value);
                        else
                            editorEl.innerHTML = newProps.value;
                    }
                }
            },
            unmount() {
