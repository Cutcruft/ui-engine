...
function h(tag: string, props: Record<string, string>, children: string = ""): HTMLElement {
...
const Button: ComponentHandle = {
  mount(container, props, onEvent) {
    ...
    if (props.loading === "true") btn.classList.add("p-button-loading");
    if (props.disabled === "true") btn.disabled = true;
    ...
    btn.addEventListener("click", () => {
      if (props.onClick) onEvent(props.onClick);
      if (props.on) onEvent(props.on);
    });
...
// InputText
const InputText: ComponentHandle = {
  mount(container, props, onEvent) {
    ...
    inp.addEventListener("input", (e) => {
      ...
      if (props.onInput) onEvent(props.onInput.replace("$event", v));
      if (props.onChange) onEvent(props.onChange.replace("$event", v));
    });
...
// Checkbox
const Checkbox: ComponentHandle = {
  mount(container, props, onEvent) {
    ...
    box.addEventListener("click", () => {
      update(!inp.checked);
      if (props.onChange) onEvent(props.onChange.replace("$event", String(inp.checked)));
    });
...
const PrimeVueComponents: Record<string, ComponentHandle> = {
...
function registerPrimeVue() {
  ...
  Object.entries(PrimeVueComponents).forEach(([name, comp]) => {
    (window as any).UIEngineModules[name] = comp;
    (window as any).UIEngineModules[name.toLowerCase()] = comp;
  });
  if ((window as any).UIEngine) {
    Object.keys(PrimeVueComponents).forEach(name => {
      (window as any).UIEngine.registerComponent(name, PrimeVueComponents[name]);
    });
  }
}
...
if (typeof window !== "undefined") {
  // ждём UIEngine
  const tryReg = () => {
    if (window.UIEngine) registerPrimeVue();
