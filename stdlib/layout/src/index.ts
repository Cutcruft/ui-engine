...
function injectFlexCSS() {
  if (document.querySelector("#primeflex-css")) return;
...
function mkFlex(tag: string, baseStyle: string): ComponentHandle {
  return {
    mount(container, props) {
      ...
      if (props.align) style += `align-items:${props.align};`;
      if (props.justify) style += `justify-content:${props.justify};`;
      if (props.wrap) style += `flex-wrap:${props.wrap};`;
      ...
      return {
        update(newProps) {
          let s = baseStyle;
          if (newProps.gap) s += `gap:${newProps.gap};`;
...
const Row: ComponentHandle = mkFlex("div", "display:flex;flex-direction:row;");
const Col: ComponentHandle = mkFlex("div", "display:flex;flex-direction:column;");
const Grid: ComponentHandle = {
  mount(container, props) {
    injectFlexCSS();
...
const Stack: ComponentHandle = mkFlex("div", "display:flex;flex-direction:column;");
const Container: ComponentHandle = {
  mount(container, props) {
    const el = document.createElement("div");
...
const LayoutComponents: Record<string, ComponentHandle> = {
  Row, row: Row,
  Col, col: Col,
  Grid, grid: Grid,
  Stack, stack: Stack,
...
if (typeof window !== "undefined") {
  const tryReg = () => {
    if (window.UIEngine) registerLayout();
